...
export const HeroFareCalculator: React.FC<HeroFareCalculatorProps> = ({
  ...
  return (
    <div id="estimator" className="relative bg-zinc-950 text-white overflow-hidden py-10 lg:py-16">
      {/* Hero Header Static Scenic Ooty Hills Background Image Layer */}
      <div 
        className="absolute inset-0 bg-cover bg-center opacity-30 mix-blend-luminosity pointer-events-none"
        style={{ backgroundImage: `url('https://images.unsplash.com/photo-1589182373726-e4f658ab50f0?auto=format&fit=crop&w=2000&q=80')` }}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          <div className="inline-flex items-center gap-2 bg-amber-500/15 border border-amber-500/30 text-amber-300 px-4 py-1.5 rounded-full text-xs sm:text-sm font-semibold mb-4">
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl shadow-2xl p-4 sm:p-8 backdrop-blur-md">
          {/* Trip Type Selector Tabs */}
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 bg-slate-950/80 p-2 rounded-xl border border-slate-800/80 mb-6">
            <button
            ...
            <button
              ...
              className="col-span-2 sm:col-span-1 flex flex-col sm:flex-row items-center justify-center gap-1.5 py-2.5 px-3 rounded-lg text-xs sm:text-sm font-bold bg-emerald-900/60 hover:bg-emerald-900 text-emerald-300 border border-emerald-500/40 transition"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end mb-6">
              ...
              <div className="md:col-span-5 space-y-1.5">
                <label className="text-xs font-semibold text-amber-400 flex items-center justify-between">
                  <span className="flex items-center gap-1">
                    <Clock className="w-3.5 h-3.5" /> Local Hours Package Rate
                  </span>
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {[2, 4, 8, 12].map((h) => (
                    <button
                      ...
                      className={`py-2 px-1 rounded-lg text-xs font-bold border transition text-center ${
                        localHours === h
                ...
                <div className="flex items-center gap-2 mt-1 bg-zinc-950 border border-zinc-800 rounded-lg p-1.5">
                  <span className="text-xs text-zinc-400 font-medium pl-1">Custom Hours:</span>
                  <input
                    type="number"
                    min="1"
                    max="24"
                    ...
                    className="w-16 bg-zinc-900 border border-zinc-700 rounded px-2 py-0.5 text-xs text-white text-center font-bold"
                  />
              ...
              <div className="md:col-span-5 space-y-1.5">
                <div className="flex justify-between items-center mb-1">
                  <button
                    ...
                    className="text-[11px] text-amber-400 hover:underline font-medium ml-auto"
                  >
                ...
                {useCustomKm ? (
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-amber-400">Custom Distance (KM)</label>
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2.5 text-sm font-medium text-white focus:outline-none focus:border-amber-400 shadow-inner"
                      />
            ...
            {tripType === 'round-trip' ? (
              <div className="md:col-span-3 space-y-1.5">
                ...
                <div className="flex items-center bg-slate-950 border border-slate-700 rounded-lg p-1">
                  <button
                    ...
                    className="px-3 py-1 bg-slate-800 text-white rounded font-bold hover:bg-slate-700"
                  >
                    -
                  </button>
              ...
              <div className="md:col-span-3 bg-slate-950/60 border border-slate-800 rounded-lg p-2.5 flex items-center justify-between">
                <div>
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 line-clamp-1 mb-2">{v.models}</p>
                      <div className="flex items-center gap-2 text-[11px] text-slate-300 font-medium mb-3">
                        <span className="bg-slate-800 px-1.5 py-0.5 rounded">👥 {v.passengers} Seats</span>
                        <span className="bg-slate-800 px-1.5 py-0.5 rounded">🧳 {v.luggage} Bags</span>
                      </div>
                    </div>
          ...
          <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 border border-slate-800 rounded-xl p-4 sm:p-5 mb-6">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div className="space-y-2 flex-1">
                ...
                <p className="text-xs text-slate-300">{fareBreakdown.notes}</p>

                {tripType !== 'local' && tripType !== 'airport' ? (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs pt-1">
                    <div className="bg-slate-900/90 p-2 rounded border border-slate-800">
                      <span className="text-slate-400 block text-[10px]">Base Route Fare</span>
                      <span className="font-bold text-white">₹{fareBreakdown.baseFare.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="bg-slate-900/90 p-2 rounded border border-slate-800">
                      <span className="text-slate-400 block text-[10px]">Driver Batta / Hill</span>
                      <span className="font-bold text-white">₹{fareBreakdown.driverBata.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="bg-slate-900/90 p-2 rounded border border-slate-800">
                      <span className="text-slate-400 block text-[10px]">Est. Toll & Taxes</span>
                      <span className="font-bold text-white">₹{fareBreakdown.estimatedTolls.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="bg-amber-500/10 p-2 rounded border border-amber-500/30">
                      <span className="text-amber-300 block text-[10px]">Final Total Fare</span>
                      <span className="font-extrabold text-amber-400 text-sm">₹{fareBreakdown.totalFare.toLocaleString('en-IN')}</span>
                    </div>
                  </div>
                ) : (
                  <div className="bg-amber-400/10 border border-amber-400/30 rounded-xl p-3 text-xs text-amber-200">
              ...
              <div className="shrink-0 flex flex-col sm:flex-row lg:flex-col gap-2">
                <button
                  ...
                  className="w-full sm:w-auto bg-gradient-to-r from-amber-400 via-amber-300 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-slate-950 font-extrabold text-base px-6 py-3.5 rounded-xl shadow-lg shadow-amber-500/25 flex items-center justify-center gap-2 transition cursor-pointer"
                >
          ...
          {/* Key Value Props Bar */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-4 border-t border-slate-800 text-xs text-slate-300">
            <div className="flex items-center gap-2">
